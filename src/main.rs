...
enum Commands {
    ...
    Index,
    /// Display a custom file of the current (or specified) year
    Page {
        /// The name of the file (e.g., notes.md)
        name: String,
        /// Specify the year (defaults to current year)
        #[arg(long)]
        year: Option<i32>,
    },
...
fn main() {
    ...
    match &cli.command {
        ...
        Commands::Page { name, year } => {
            let notes_folder = get_notes_folder();

            // Determine year: User provided OR current year
            let target_year = year.unwrap_or_else(|| Local::now().year());

            let file_path = Path::new(&notes_folder)
                .join(target_year.to_string())
                .join(name);

            // Ensure parent directory exists (e.g., the 2025 folder)
            if let Some(parent) = file_path.parent() {
                fs::create_dir_all(parent).expect("Unable to create directory");
            }

            // Create file if it does not exist so the editor has something to open
            if !file_path.exists() {
                fs::write(&file_path, "").expect("Unable to create file");
